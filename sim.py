...
col_row_names = ["0-0","0-15","15-0","15-15","30-0","0-30","40-0","30-15","15-30","0-40","40-15","15-40","30-30(DEUCE)","40-30(A-40)","30-40(40-A)","Win", "Lose"]
...
col_row_names3 = ["0-0","0-1","1-0","1-1",
                     ...
                     "4-5", "5-5","6-5","5-6",
                     "6-6","SETv1","SETv2"]
col_row_names4 = ["0-0","0-1","1-0","1-1","2-0","0-2","2-1","1-2","V1","V2"]
...
def predict1(gamescore, pwin1, pwin2, ptie1, pset_v1, match_initial_state, set_initial_sate, game_initial_state, tb_initial_state):
...
def predict2(setscore, gamescore, pwin1, pwin2, ptie1, pset_v1, match_initial_state, set_initial_sate, game_initial_state, tb_initial_state):
    s1match = match_initial_state
    s1set = set_initial_sate
    s1game = game_initial_state
    s1tb = tb_initial_state
    ...
    if (setscore == "0-1"):
        ...
        ans = prob_set( pwin1, pwin2, ptie1, s1set)
        ans = pd.DataFrame(data = ans, columns=col_row_names3)
        temp1= ans.at[0,"SETv1"]

        s1match.at[0,"0-2"] = temp
        s1match.at[0,"1-1"] = temp1
        s1match.at[0, "0-1"] = 0
    ...
    return prob_entire_match
def predict3(gamescore, pwin1, pwin2, ptie1, pset_v1, match_initial_state, set_initial_sate, game_initial_state, tb_initial_state):
    s1match = match_initial_state
    s1set = set_initial_sate
    s1game = game_initial_state
    s1tb = tb_initial_state
...
def tennis_model(ppoint_srv1, ppoint_srv2, setscore, gamescore, match_initial_state, set_initial_sate, game_initial_state, tb_initial_state):
    ...
    if(setscore == "0-0"):
        prob_entire_match = predict1(gamescore,temp,temp1,temp2,temp3,match_initial_state,set_initial_sate,game_initial_state,tb_initial_state)
    if setscore == "1-1":
        prob_entire_match = predict3(gamescore,temp,temp1,temp2,temp3,match_initial_state,set_initial_sate,game_initial_state,tb_initial_state)
    if setscore == "1-0" or setscore == "0-1":
        print("yes")
        prob_entire_match = predict2(setscore,gamescore,temp,temp1,temp2,temp3,match_initial_state,set_initial_sate,game_initial_state,tb_initial_state)
    return prob_entire_match
...
ans = pd.DataFrame(data = ans, columns=col_row_names)#
temp = ans.at[0,"Win"]
ans = prob_game(0.6,game_initial_state)
ans = pd.DataFrame(data = ans, columns=col_row_names)
temp1 = ans.at[0,"Win"]
ans = prob_tie(0.7,0.6,tb_initial_state)
ans = pd.DataFrame(data = ans, columns=col_row_names2)
temp2 = ans.at[0,"SETv1"]
ans = prob_set(temp,temp1,temp2,set_initial_sate)
ans = pd.DataFrame(data = ans, columns=col_row_names3)
ans = prob_match(0.7,match_initial_state)
ans = pd.DataFrame(data = ans, columns=col_row_names4)
ans = tennis_model(0.7,0.6,"1-0","0-3",match_initial_state,set_initial_sate,game_initial_state,tb_initial_state)
ans = pd.DataFrame(data = ans, columns=col_row_names4)
