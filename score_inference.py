...
def get_score_time_series(p, q, df_odds, server):
    ...
    for i, odds in enumerate(odds_arr):
        game_temp = gamescore
        setscore, gamescore, p, q = calc_score(p, q, setscore, gamescore, server, odds, set_trans, match_trans, mis,
                                               sis, gis, tbis)
    ...
    df = pd.DataFrame(
        {'r1_gs': r1_gamescore, 'r2_gs': r2_gamescore, 'r1_setscore': r1_setscore, 'r2_setscore': r2_setscore},
        index=df_odds.index)
    df.iloc[i + 1:] = 0
...
def calc_score(p, q, setscore, gamescore, server, implied_odds, set_transitions, match_transitions, mis, sis, gis,
               tbis):
    ...
    if nextmatchv1 == ('3-2') and nextmatchv2 == ('2-3'):
        ...
        m1 = 1
        m2 = markov.tennis_model(p, q, nextmatchv2, nextsetv2, mis2, sis2, gis2, tbis2)['r1_win'].values[0]
    elif nextmatchv2 == '2-3' or nextmatchv2 == '1-3' or nextmatchv2 == '0-3':
        m1 = markov.tennis_model(p, q, nextmatchv1, nextsetv1, mis1, sis1, gis1, tbis1)['r1_win'].values[0]
        m2 = 0
    else:
        m1 = markov.tennis_model(p, q, nextmatchv1, nextsetv1, mis1, sis1, gis1, tbis1)['r1_win'].values[0]
        m2 = markov.tennis_model(p, q, nextmatchv2, nextsetv2, mis2, sis2, gis2, tbis2)['r1_win'].values[0]
    ...
    print(p, q, m1, m2, implied_odds, setscore, gamescore)
